...
void frg_ast_visitor_add_entry(
    ...
    visitor->entries[id].callback_pre = callback_pre;
    visitor->entries[id].callback_post = callback_post;
}
...
frg_ast_visitor_status_t _frg_ast_accept_recursive(
    ...
    if (visitor->entries[(*ast)->id].callback_pre != NULL) {
        frg_ast_visitor_status_t status = visitor->entries[(*ast)->id].callback_pre(
            parents,
            ast,
            visitor->user_data
        );
    ...
    frg_ast_visitor_status_t status = FRG_AST_VISITOR_STATUS_OK;
    ...
    if (visitor->entries[(*ast)->id].callback_post != NULL) {
        frg_ast_visitor_status_t status = visitor->entries[(*ast)->id].callback_post(
            parents,
            ast,
            visitor->user_data
        );
