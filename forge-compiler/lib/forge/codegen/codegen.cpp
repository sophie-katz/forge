...
void frg_codegen_module_print(frg_stream_output_t* mut_stream,
  ...
  frg_stream_output_write_string(mut_stream, llvm_ir_string.c_str());
}
...
bool frg_codegen_module_write_object_file(frg_message_buffer_t* mut_message_buffer,
  ...
  frg_message_emit(mut_message_buffer,
  ...
  if (target == NULL) {
    frg_message_emit_fg_2_invalid_target_triple(
      mut_message_buffer, target_triple.c_str(), error_message.c_str());
  ...
  llvm::raw_fd_ostream stream(path, error_code, llvm::sys::fs::OF_None);

  if (error_code) {
    frg_message_emit_ffs_1_open_for_writing(
      mut_message_buffer, path, error_code.message().c_str());
  ...
  if (target_machine->addPassesToEmitFile(
    ...
    frg_message_emit_fg_1_cannot_emit_object_file(mut_message_buffer,
                                                  target_triple.c_str());
...
frg_codegen_module_t* frg_codegen(const frg_ast_node_t* node) {
  ...
  frg_codegen_module_t* module
    = (frg_codegen_module_t*)frg_malloc(sizeof(frg_codegen_module_t));
  ...
  const frg_ast_node_declaration_block_t* declaration_block
    = (const frg_ast_node_declaration_block_t*)node;

  frg_ast_scope_t* scope = frg_ast_scope_new();

  frg_ast_scope_load_declaration_block(scope, declaration_block);
  ...
  for (const GList* it = declaration_block->declarations; it != NULL; it = it->next) {
    ...
    case FRG_AST_NODE_KIND_DECLARATION_FUNCTION:
      _frg_codegen_generate_declaration_function(*module->llvm_builder,
                                                 *module->llvm_ctx,
                                                 *module->llvm_module,
                                                 scope,
                                                 declaration);
