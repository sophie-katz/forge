...
frg_ast_node_t* frg_parse(
    ...
    if (frg_stream_input_is_buffer(mut_source->stream)) {
        frg_assert_int_greater_than_or_equal_to(frg_stream_get_length(mut_source->stream), 2);
        frg_assert_int_equal_to(
            frg_stream_get_buffer(mut_source->stream)[frg_stream_get_length(mut_source->stream) - 2],
            0
        );
        frg_assert_int_equal_to(
            frg_stream_get_buffer(mut_source->stream)[frg_stream_get_length(mut_source->stream) - 1],
            0
        );

        // Set the scan buffer
        yy_scan_buffer(
            frg_stream_get_buffer(mut_source->stream),
