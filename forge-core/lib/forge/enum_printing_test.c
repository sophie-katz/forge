...
void test_ast_node_type_int_flags() {
  ...
  for (frg_ast_node_type_int_flags_t value = 1; value <= FRG_AST_NODE_TYPE_INT_FLAG_MAX;
       value <<= 1) {
...
void test_ast_node_declaration_property_flags() {
  ...
  frg_ast_node_declaration_property_flags_print(
    stream, FRG_AST_NODE_DECLARATION_PROPERTY_FLAG_NONE);
  GString* buffer = frg_stream_output_destroy_take_buffer(stream);
  TEST_ASSERT_EQUAL_STRING("none", buffer->str);
  g_string_free(buffer, TRUE);

  for (frg_ast_node_declaration_property_flags_t value = 1;
       value <= FRG_AST_NODE_DECLARATION_PROPERTY_FLAG_MAX;
       value <<= 1) {
    stream = frg_stream_output_new_buffer(FRG_STREAM_OUTPUT_FLAG_NONE);
...
void test_ast_node_declaration_interface_flags() {
  ...
  frg_ast_node_declaration_interface_flags_print(
    stream, FRG_AST_NODE_DECLARATION_INTERFACE_FLAG_NONE);
  GString* buffer = frg_stream_output_destroy_take_buffer(stream);
  TEST_ASSERT_EQUAL_STRING("none", buffer->str);
  g_string_free(buffer, TRUE);

  for (frg_ast_node_declaration_interface_flags_t value = 1;
       value <= FRG_AST_NODE_DECLARATION_INTERFACE_FLAG_MAX;
       value <<= 1) {
    stream = frg_stream_output_new_buffer(FRG_STREAM_OUTPUT_FLAG_NONE);
...
void test_ast_node_declaration_function_argument_flags() {
  ...
  frg_ast_node_declaration_function_argument_flags_print(
    stream, FRG_AST_NODE_DECLARATION_FUNCTION_ARGUMENT_FLAG_NONE);
  GString* buffer = frg_stream_output_destroy_take_buffer(stream);
  TEST_ASSERT_EQUAL_STRING("none", buffer->str);
  g_string_free(buffer, TRUE);

  for (frg_ast_node_declaration_function_argument_flags_t value = 1;
       value <= FRG_AST_NODE_DECLARATION_FUNCTION_ARGUMENT_FLAG_MAX;
       value <<= 1) {
    stream = frg_stream_output_new_buffer(FRG_STREAM_OUTPUT_FLAG_NONE);
...
void test_ast_node_declaration_function_flags() {
  ...
  frg_ast_node_declaration_function_flags_print(
    stream, FRG_AST_NODE_DECLARATION_FUNCTION_FLAG_NONE);
  GString* buffer = frg_stream_output_destroy_take_buffer(stream);
  TEST_ASSERT_EQUAL_STRING("none", buffer->str);
  g_string_free(buffer, TRUE);

  for (frg_ast_node_declaration_function_flags_t value = 1;
       value <= FRG_AST_NODE_DECLARATION_FUNCTION_FLAG_MAX;
       value <<= 1) {
    stream = frg_stream_output_new_buffer(FRG_STREAM_OUTPUT_FLAG_NONE);
