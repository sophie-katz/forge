...
void frg_formatting_formatted_printer_type_int(
    ...
    frg_stream_output_write_printf(
        mut_stream,
        "%c%u",
        (((const frg_ast_node_type_int_t*)node)->flags & FRG_AST_NODE_TYPE_INT_FLAG_UNSIGNED) != 0 ? 'u' : 'i',
...
void frg_formatting_formatted_printer_type_float(
    ...
    frg_stream_output_write_printf(
        mut_stream,
        "f%u",
        ((const frg_ast_node_type_float_t*)node)->bit_width
    );
}
...
void frg_formatting_formatted_printer_type_symbol(
    ...
    frg_stream_output_write_string(mut_stream, ((const frg_ast_node_type_symbol_t*)node)->name->str);
}
...
void frg_formatting_formatted_printer_type_pointer(
    ...
    frg_formatting_print_formatted(mut_stream, ((const frg_ast_node_type_pointer_t*)node)->value, indentation);
}
...
void frg_formatting_formatted_printer_type_function(
    ...
    frg_assert_pointer_null(((const frg_ast_node_type_function_t*)node)->variadic_positional_arguments);
    frg_assert_pointer_null(((const frg_ast_node_type_function_t*)node)->variadic_keyword_arguments);
    frg_assert_pointer_non_null(((const frg_ast_node_type_function_t*)node)->return_type);

    frg_stream_output_write_character(mut_stream, '(');

    for (GList* iter = ((const frg_ast_node_type_function_t*)node)->arguments; iter != NULL; iter = iter->next) {
    ...
    frg_formatting_print_formatted(mut_stream, ((const frg_ast_node_type_function_t*)node)->return_type, indentation);
}
...
void frg_formatting_formatted_printer_declaration_property(
    ...
    if ((((const frg_ast_node_declaration_property_t*)node)->flags & FRG_AST_NODE_DECLARATION_PROPERTY_FLAG_SPREAD) != 0) {
        frg_stream_output_write_string(mut_stream, "...");
    }

    frg_stream_output_write_string(mut_stream, ((const frg_ast_node_declaration_property_t*)node)->name->str);

    if ((((const frg_ast_node_declaration_property_t*)node)->flags & FRG_AST_NODE_DECLARATION_PROPERTY_FLAG_OPTIONAL) != 0) {
        frg_stream_output_write_character(mut_stream, '?');
    } else if ((((const frg_ast_node_declaration_property_t*)node)->flags & FRG_AST_NODE_DECLARATION_PROPERTY_FLAG_NON_OPTIONAL) != 0) {
        frg_stream_output_write_character(mut_stream, '!');
    }

    if (((const frg_ast_node_declaration_property_t*)node)->type != NULL) {
        frg_stream_output_write_string(mut_stream, ": ");
        frg_formatting_print_formatted(mut_stream, ((const frg_ast_node_declaration_property_t*)node)->type, indentation);
    }
}
...
void frg_formatting_formatted_printer_declaration_function_argument(
    ...
    if ((((frg_ast_node_declaration_function_argument_t*)node)->flags & FRG_AST_NODE_DECLARATION_FUNCTION_ARGUMENT_FLAG_KEYWORD) != 0) {
        frg_stream_output_write_string(mut_stream, "kw ");
    }

    frg_formatting_print_formatted(mut_stream, (frg_ast_node_t*)((frg_ast_node_declaration_function_argument_t*)node)->property, indentation);

    if (((frg_ast_node_declaration_function_argument_t*)node)->default_value != NULL) {
        frg_stream_output_write_string(mut_stream, " = ");
        frg_formatting_print_formatted(mut_stream, ((frg_ast_node_declaration_function_argument_t*)node)->default_value, indentation);
    }
}
