...
void _callback_return_true(const frg_message_buffer_t* message_buffer,
                           void* mut_shared_library) {
  TEST_ASSERT_EQUAL(0, message_buffer->warning_count);
  TEST_ASSERT_EQUAL(0, message_buffer->error_count);

  bool (*return_true)()
    = frg_test_compilation_get_function(mut_shared_library, "return_true");

  TEST_ASSERT(return_true());
}

void test_return_true() {
  frg_test_compilation("return_true",
                       // Source
                       "fn return_true() -> bool {\n"
                       "  return true;\n"
                       "}\n",
...
void _callback_return_false(const frg_message_buffer_t* message_buffer,
                            void* mut_shared_library) {
  TEST_ASSERT_EQUAL(0, message_buffer->warning_count);
  TEST_ASSERT_EQUAL(0, message_buffer->error_count);

  bool (*return_false)()
    = frg_test_compilation_get_function(mut_shared_library, "return_false");

  TEST_ASSERT_FALSE(return_false());
}

void test_return_false() {
  frg_test_compilation("return_false",
                       // Source
