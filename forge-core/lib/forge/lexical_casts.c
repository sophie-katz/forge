...
void _frg_print_char_unquoted(frg_stream_output_t* mut_stream,
...
size_t _frg_parse_uint_partial(uint64_t* out_value,
...
bool _frg_parse_char_unquoted(frg_character_t* out_value,
  ...
  if (frg_parsing_token_reader_get_current_character(mut_reader) == '\\') {
    ...
    if (frg_parsing_token_reader_get_current_character(mut_reader) == 0) {
      ...
      frg_message_emit_es_3_string_ends_with_backslash(mut_message_buffer,
                                                       &source_range);
      ...
      *out_value = '\0';
      ...
      size_t digit_count
        = _frg_parse_uint_partial(&value_uncasted, mut_reader, 16, 2, false);
      ...
      *out_value = 0;
      ...
      size_t digit_count
        = _frg_parse_uint_partial(&value_uncasted, mut_reader, 16, 0, true);
...
bool frg_parse_character(frg_character_t* out_value,
  ...
  GString* value_string
    = _frg_parse_string_with_quote_character(mut_message_buffer, mut_reader, '\'');
...
bool frg_parse_string(GString** out_value,
  ...
  *out_value = _frg_parse_string_with_quote_character(mut_message_buffer, reader, '"');

  return *out_value != NULL;
}

void _frg_print_base_prefix(frg_stream_output_t* mut_stream, frg_int_base_t base) {
  if (base == 2) {
...
void _frg_print_uint_without_base_prefix(frg_stream_output_t* mut_stream,
                                         uint64_t value,
                                         frg_int_base_t base) {
  frg_assert(frg_is_valid_base(base));
...
void frg_print_uint(frg_stream_output_t* mut_stream,
  ...
  _frg_print_uint_without_base_prefix(mut_stream, value, base);
}
...
void frg_print_uint_with_suffix(frg_stream_output_t* mut_stream,
  ...
  frg_print_uint(mut_stream, value, base);
...
void _frg_parse_base_prefix(frg_int_base_t* out_base,
                            frg_message_buffer_t* mut_message_buffer,
                            frg_parsing_token_reader_t* mut_reader) {
...
bool frg_parse_uint(frg_parse_uint_result_t* out_result,
  ...
  size_t digit_count
    = _frg_parse_uint_partial(&out_result->value, mut_reader, base, 0, true);
  ...
  if (digit_count == 0) {
    ...
    size_t digit_count
      = _frg_parse_uint_partial(&bit_width_uncasted, mut_reader, 10, 0, false);
...
void _frg_print_float_without_scientific_notation(frg_stream_output_t* mut_stream,
                                                  frg_f64_t value,
                                                  frg_int_base_t base) {
  frg_assert(frg_is_valid_base(base));
...
void frg_print_float(frg_stream_output_t* mut_stream,
  ...
  if (scientific_notation) {
    ...
    _frg_print_float_without_scientific_notation(
      mut_stream, value * negative_factor, base);

    frg_stream_output_write_character(mut_stream, 'e');

    _frg_print_uint_without_base_prefix(mut_stream, exponent, base);
  } else {
...
void frg_print_float_with_suffix(frg_stream_output_t* mut_stream,
  ...
  frg_print_float(mut_stream, value, base, scientific_notation);
