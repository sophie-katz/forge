...
GList* _frg_linking_lld_create_arguments(
    ...
    for (const GList* objects_iter = objects; objects_iter != NULL; objects_iter = objects_iter->next) {
        arguments = g_list_append(
            arguments,
            g_string_new_len(((GString*)objects_iter->data)->str, ((GString*)objects_iter->data)->len)
        );
    }
...
bool _frg_linking_link_lld(
    ...
    GList* arguments = _frg_linking_lld_create_arguments(
        mode,
        output_path,
        objects
    );
...
bool frg_link(
    ...
    switch (frg_linking_configuration_get_current_linker_kind(configuration)) {
        ...
        default:
            frg_die_unexpected_enum_value(frg_linking_configuration_get_current_linker_kind(configuration));
    }
}
