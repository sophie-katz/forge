...
bool _frg_log_event(
    ...
    if (frg_global_log_enabled) {
        frg_stream_output_set_color(frg_log_stream_get(), FRG_STREAM_OUTPUT_COLOR_BRIGHT_BLACK);
        frg_stream_output_write_printf(frg_log_stream_get(), "[%s:%i:%s] ", file, line_number, function);
