...
void _frg_message_print_prefix_internal(
    ...
    if (_frg_message_should_print_prefix_internal(severity)) {
        ...
        frg_stream_output_write_printf(mut_stream, "[internally from forge-core/%s:%d]\n", log_path_resolved, log_line_number);
    ...
    if (source_location->path != NULL) {
        ...
        if (source_location->line_number != 0) {
            ...
            if (source_location->column_number != 0) {
                frg_stream_output_write_printf(mut_stream, "%i:", source_location->column_number);
            }
        } else {
            frg_assert_int_equal_to(source_location->column_number, 0);
        }
...
void _frg_message_print_prefix_severity_and_code(
    frg_stream_output_t* mut_stream,
    frg_message_severity_t severity,
    const char* code
) {
    ...
    switch (severity) {
        ...
        case FRG_MESSAGE_SEVERITY_INTERNAL_ERROR:
            frg_stream_output_set_color(mut_stream, FRG_STREAM_OUTPUT_COLOR_CYAN);
            frg_stream_output_set_color(mut_stream, FRG_STREAM_OUTPUT_COLOR_BOLD);
            frg_stream_output_write_printf(mut_stream, "internal error%s ", code == NULL ? ":" : "");
            frg_stream_output_set_color(mut_stream, FRG_STREAM_OUTPUT_COLOR_RESET);
            break;
        default:
...
const char* _frg_message_get_line_number_separator(
    frg_stream_output_t* mut_stream
) {
    frg_assert_pointer_non_null(mut_stream);

    return frg_stream_output_choose_ascii_or_unicode(
        mut_stream,
        "|",
        "▏"
    );
...
void _frg_message_print_sample_prefix_line_number(
    ...
    frg_stream_output_write_printf(
        ...
        "%i %s ",
        line_number,
        _frg_message_get_line_number_separator(
            mut_stream
        )
    );
...
void _frg_message_print_sample(
    ...
    if (_frg_message_should_print_sample(message)) {
        ...
        ssize_t text_offset = message->source_range.start.offset - (message->source_range.start.column_number - 1);
