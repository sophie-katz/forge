...
int _frg_configuration_command_callback_version(
  ...
  if (options->version_short) {
    frg_cli_program_print_version_short(frg_stream_output_get_stdout(),
                                        (frg_cli_program_t*)program);
  } else {
    frg_cli_program_print_version_long(frg_stream_output_get_stdout(),
                                       (frg_cli_program_t*)program);
  }
...
frg_cli_command_t* frg_configuration_new_command_version() {
  ...
  frg_cli_option_set_add_option(
    ...
    frg_cli_option_new_flag(
      "short",
      "Use short format (<major>.<minor>.<patch>-<label>). This is intended for "
      "scripting use.",
