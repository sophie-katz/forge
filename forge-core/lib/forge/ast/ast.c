...
frg_status_t frg_ast_new_value_i8(
    ...
    if (result != FRG_STATUS_OK) {
        frg_log_internal_error("unable to create primary type: %s", frg_status_to_string(result));
        return result;
    }
...
frg_status_t frg_ast_destroy(frg_ast_t** ast) {
    ...
    switch ((*ast)->id) {
        ...
        case FRG_AST_ID_DECL_UNION:
            ...
            if (result != FRG_STATUS_OK) {
                frg_log_internal_error("unable to destroy AST list: %s", frg_status_to_string(result));
                return result;
            }
        ...
        case FRG_AST_ID_DECL_PROP:
            ...
            if (result != FRG_STATUS_OK) {
                frg_log_internal_error("unable to destroy AST: %s", frg_status_to_string(result));
                return result;
            }
